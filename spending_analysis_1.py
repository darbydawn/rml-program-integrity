"""
RML Federal Spending Analysis -- Phase 6
Rocky Mountain Laboratories Program Integrity Investigation
Author: Dawn Krysa, PA-C, MSHIIM, CHDA
Date: June 2026

Analyzes USASpending.gov contract and assistance data for ZIP 59840
(Hamilton, MT) to identify program integrity flags related to the
June 2, 2026 federal criminal charges against NIH researchers
...
contracts = load_csv('../data/contracts_rml_59840.csv')
assistance = load_csv('../data/assistance_rml_59840.csv')
...
print("=" * 65)
print("FINANCIAL OVERVIEW -- ZIP 59840 (HAMILTON, MT)")
print("=" * 65)
...
print(f"\nCOMBINED FEDERAL FOOTPRINT IN ZIP 59840")
...
print("\n" + "=" * 65)
print("FLAG 1: CARES ACT FUNDS FOR CONSTRUCTION")
print("=" * 65)
...
for r in sorted(covid_contracts, key=lambda x: safe_float(x['obligated_amount_from_COVID-19_supplementals']), reverse=True):
    ...
    if covid_amt > 1_000_000 and '236' in str(r.get('naics_code','')):
        print(f"  *** HIGH PRIORITY FLAG: CARES Act emergency funds used for construction ***")
    print()
...
# ─────────────────────────────────────────────
# FLAG 3: CAPE FOX ANC SET-ASIDE
# ─────────────────────────────────────────────

print("\n" + "=" * 65)
print("FLAG 3: CAPE FOX FACILITIES SERVICES -- ANC CONTRACT ANALYSIS")
print("=" * 65)

cape_rows = [r for r in contracts if 'CAPE FOX' in str(r.get('recipient_name',''))]
for r in cape_rows:
    obl = safe_float(r['total_obligated_amount'])
    ...
    print(f"  Change: Prior contract $2.4M, current contract $18M -- 7.5x increase")
...
print("\n" + "=" * 65)
print(f"FLAG 5: NIH CONTRACTS ACTIVE AFTER ARREST DATE ({ARREST_DATE})")
print("=" * 65)

active_post_arrest = [r for r in nih_contracts
                      if r.get('period_of_performance_current_end_date','') >= ARREST_DATE
                      and safe_float(r['total_obligated_amount']) > 500_000]

active_total = sum(safe_float(r['total_obligated_amount']) for r in active_post_arrest)
print(f"\nContracts >$500K still active post-arrest: {len(active_post_arrest)}")
print(f"Total obligated: ${active_total:,.0f}")
print(f"No documented pause, modification, or review found in data\n")

for r in sorted(active_post_arrest, key=lambda x: safe_float(x['total_obligated_amount']), reverse=True)[:12]:
    obl = safe_float(r['total_obligated_amount'])
    end = r.get('period_of_performance_current_end_date','')
...
FLAG 1 -- CARES Act construction:
  ...
  Potential false certification of COVID emergency use
...
FLAG 4 -- Contractor concentration:
  Jackson Contractor Group: 104 contracts, $55M (8.7% of NIH spend)
  Warrants competitive bidding compliance review

FLAG 5 -- Active contracts post-arrest:
  13 contracts, $142M active after Jan 25, 2026
  No evidence of pause or review following criminal charges
  Largest: Hensel Phelps $127.5M, active through Oct 2027
""")
